...
> With centralized wastewater treatment, wastewater is piped through a collection system to a centralized facility, which treats the water through a variety of different processes prior to discharge. Centralized facilities are maintained and upgraded by a city, sewer district, or other organization, which collects a bill from users attached to the wastewater system.
...
* **Treatment Process** 
  ...
  + **Disinfection** - Disinfection is used to kill potentially harmful pathogens in the water. Common methods of disinfection include chlorination and ultraviolet light.  
...
* **Land Application**
  ...
  + **Irrigation** - Treated wastewater is commonly used for irrigation. It must be treated before irrigation to remove harmful bacteria and other water quality issues, especially if it is to be used on agricultural areas. 

  + **Biosolids Application** - Biosolids must be removed from the clarifiers periodically to allow for continued treatment of wastewater. Some facilities have approved plans to land apply these biosolids as fertilizer. For facilities that discharge to surface waters, this is included in the NPDES permit. For those that do not discharge to surface waters, state administered WPCF permits can be obtained through the Oregon DEQ.
...
> Transition to centralized systems is driven by population density, cost, and environmental concerns. Once communities reach a critical population density, septic systems may start to cause environmental problems or there may be limited space for placing additional systems. At this point it becomes more practical and perhaps cost-effective to transition to centralized treatment. This eliminates space concerns, as collection will be more cost-effective the closer together buildings are spaced. It also reduces environmental impacts by implementing centralized treatment that requires a permit and must meet pollution discharge requirements. Regular maintenance ensures that wastewater is properly treated to permit standards, whereas individual septic systems may fail or be irregularly maintained.